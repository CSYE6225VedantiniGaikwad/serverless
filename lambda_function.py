    ...
    try:
        if status == "SUCCESS":
            ...
            send_email(user_email, submission_url, assignment_id, source_email, "Submission Received - Canvas",
                       "We are pleased to inform you that your submission for the assignment and has been "
        ...
        send_email(user_email, submission_url, assignment_id, source_email, "Submission Error - Canvas",
                   "There was an error with your submission. Please ensure the URL is correct and the content is not "
                   "empty.")
...
def send_email(user_email, submission_url, assignment_id, source_email, subject, body):
    ...
    # Mailgun parameters
    logger = logging.getLogger()
    domain = "vedantinigaikwad.me"
    api_key = "8ea932e9bc44c97e48800979d9becc8e-30b58138-2dc494c9"
    ...
    email_body = (f"\n"
                  ...
                  f"     \n"
                  f"    Should you have any questions or need further assistance, please feel free to contact us at "
                  f"info@vedantinigaikwad.me.\n"
                  f" \n"
                  f"    We appreciate your effort and time.\n"
                  f" \n"
                  f"    Best regards,\n"
                  f"    The Canvas Team\n"
                  f"    ").format(user_email=user_email, submission_url=submission_url,
                                  assignment_id=assignment_id)
    # Mailgun API endpoint
    url = "https://api.mailgun.net/v3/vedantinigaikwad.me/messages"
    ...
    # Email parameters
    data = {
        "from": "mailgun <mailgun@vedantinigaikwad.me>",
        "to": to_address,
        "subject": subject,
        "text": email_body
    }
